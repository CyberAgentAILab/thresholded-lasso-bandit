...
def run_exp(num_trial, T, d, rho_sq, algs):
...
def main():
    ...
    # define algorithms
    algs = [
        (THLassoBandit, {'K': 3, 'd': d, 'lam0': 0.02})
    ]

    # run experiments
    print('===== Run experiments over {} trials ====='.format(args.num_trial))
    trajectories = run_exp(args.num_trial, T, d, rho_sq, algs)

    # save log
    for i in range(len(algs)):
        save_path = 'log/hard_instance/d{}_rho{}/{}/'.format(d, rho_sq, algs[i][0].__name__)
